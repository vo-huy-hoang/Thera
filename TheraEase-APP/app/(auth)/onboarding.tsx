...
const tips = [
	{
		icon: "lightbulb",
		text: "Luyện tập đều đặn mỗi ngày giúp giảm đau hiệu quả",
	},
...
export default function OnboardingScreen() {
	...
	const checkExistingProfile = async () => {
		try {
			...
			if (user?.onboarding_completed) {
				console.log(
					"Onboarding: Profile exists in local store, redirecting to home",
				);
	...
	const [painArea, setPainArea] = useState<"neck" | "back" | "both" | null>(
		null,
	);
	...
	const handleSubmit = async () => {
		try {
			...
			const profileData = {
				...
				diet_type: "",
				pain_areas:
					painArea === "both" ? ["neck", "back"] : painArea ? [painArea] : [],
				symptoms,
			...
			<Animated.View entering={FadeInDown.delay(300)}>
				...
				<TextInput
					...
					onChangeText={(text) => {
						...
						if (
							numericValue &&
							(parseInt(numericValue) < 1 || parseInt(numericValue) > 120)
						) {
				...
				<Animated.View entering={ZoomIn.delay(200).springify()}>
					...
					>
						<LinearGradient
							...
							style={[
								styles.optionCard,
								painArea === "neck" && styles.optionCardSelected,
							]}
						>
							<Bone
								size={width * 0.14}
								color={painArea === "neck" ? "#FFFFFF" : colors.primary}
								strokeWidth={2}
							/>
							<Text
								style={[
									styles.optionText,
									painArea === "neck" && styles.optionTextSelected,
								]}
							>
								Cổ
							</Text>
				...
				<Animated.View entering={ZoomIn.delay(300).springify()}>
					...
					>
						<LinearGradient
							...
							style={[
								styles.optionCard,
								painArea === "back" && styles.optionCardSelected,
							]}
						>
							<UserSquare2
								size={width * 0.14}
								color={painArea === "back" ? "#FFFFFF" : colors.primary}
								strokeWidth={2}
							/>
							<Text
								style={[
									styles.optionText,
									painArea === "back" && styles.optionTextSelected,
								]}
							>
								Lưng
							</Text>
				...
				<Animated.View entering={ZoomIn.delay(400).springify()}>
					...
					>
						<LinearGradient
							...
							style={[
								styles.optionCard,
								painArea === "both" && styles.optionCardSelected,
							]}
						>
							<Users
								size={width * 0.14}
								color={painArea === "both" ? "#FFFFFF" : colors.primary}
								strokeWidth={2}
							/>
							<Text
								style={[
									styles.optionText,
									painArea === "both" && styles.optionTextSelected,
								]}
							>
								Cả hai
							</Text>
			...
			<View style={styles.optionsContainer}>
				<Animated.View entering={ZoomIn.delay(200).springify()}>
					...
					>
						<LinearGradient
							colors={
								surgeryHistory === SURGERY_HISTORY.NEVER
									? ["#5B9BD5", "#4A7FB8"]
									: ["#FFFFFF", "#F5F5F5"]
							}
							start={{ x: 0, y: 0 }}
				...
				<Animated.View entering={ZoomIn.delay(400).springify()}>
					...
					>
						<LinearGradient
							colors={
								surgeryHistory === SURGERY_HISTORY.OVER_6_MONTHS
									? ["#5B9BD5", "#4A7FB8"]
									: ["#FFFFFF", "#F5F5F5"]
							}
							start={{ x: 0, y: 0 }}
